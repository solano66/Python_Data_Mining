...
# If you run this code in Tensorflow 2, please execute the following first. (It seems not working !)
# import os
...
nba_stats = nba.describe(include='all')


# ### 取得「fg.」為NaN的列資料


tmp = nba[nba['fg.'].isnull()] # fg: field goal . percent
...
kmeans_model.fit(good_columns[["age","fg","trb","pts"]]) # 注意！改good_columns
...
nba.loc[labels == 0,["player", "pos", "age","fg","trb","pts", 'ast']].head()
c1 = nba.loc[labels == 0,["player", "pos", "age","fg","trb","pts", 'ast']].describe(include='all')

# ### 第二群的前五位球員



nba.loc[labels == 1,["player", "pos", "age","fg","trb","pts", 'ast']].head()
c2 = nba.loc[labels == 1,["player", "pos", "age","fg","trb","pts", 'ast']].describe(include='all')

# ### 第三群的前五位球員



nba.loc[labels == 2,["player", "pos", "age","fg","trb","pts", 'ast']].head()
c3 = nba.loc[labels == 2,["player", "pos", "age","fg","trb","pts", 'ast']].describe(include='all')

# ### 第四群的前五位球員



nba.loc[labels == 3,["player", "pos", "age","fg","trb","pts", 'ast']].head()
c4 = nba.loc[labels == 3,["player", "pos", "age","fg","trb","pts", 'ast']].describe(include='all')

# ### 第五群的前五位球員



nba.loc[labels == 4,["player", "pos", "age","fg","trb","pts", 'ast']].head()
c5 = nba.loc[labels == 4,["player", "pos", "age","fg","trb","pts", 'ast']].describe(include='all')
...
pca_2 = PCA(2) # n_components = 2 for visualization in 2D
plot_columns = pca_2.fit_transform(good_columns)
plot_columns.shape

import matplotlib.pyplot as plt
import numpy as np
pos = list(np.unique(nba.pos)) # ['C', 'F', 'G', 'PF', 'PG', 'SF', 'SG']
colors = ['black', 'blue', 'purple', 'yellow', 'white', 'red', 'lime']
for i in range(len(colors)):
    px = plot_columns[:, 0][nba.pos == pos[i]]
    py = plot_columns[:, 1][nba.pos == pos[i]]
...
# plt.scatter(x=plot_columns[:,0], y=plot_columns[:,1], c=labels) # c: colors for the points
# plt.show()
...
ax = gca() # Get the current Axes instance on the current figure matching the given keyword args, or create one.
ax.legend();
